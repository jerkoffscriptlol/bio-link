...
const { now, normalizeSlug, clamp, sanitizeCss, get, all, run, migrate } = require("./db");
...
async function isImpersonating(req) {
  ...
  return !!(await get("SELECT 1 FROM impersonations WHERE token = ? LIMIT 1", [token]));
}
...
app.get("/health", async (req, res) => {
  const ok = !!process.env.TURSO_DATABASE_URL && !!process.env.TURSO_AUTH_TOKEN;
  res.json({ ok, hasUrl: !!process.env.TURSO_DATABASE_URL, hasToken: !!process.env.TURSO_AUTH_TOKEN });
});
...
app.post("/signup", async (req, res) => {
  ...
  try {
    const info = await run("INSERT INTO users (username, password_hash, created_at) VALUES (?, ?, ?)", [
      username,
      hash,
      now()
    ]);
